...
public class IssuesController : ControllerBase
{
        ...
        )
    {
        ...
        var response = new EmployeeIssueCommandModel
        {
            Id = Guid.NewGuid(),
    ...
    public async Task<ActionResult> GetIssueAsync(Guid id,[FromServices] IDocumentSession session)
    {
        ...
        if(response is null)
        {   
            // See if it is stored in the "temp table" - because it hasn't been processed yet
            var savedTemp = await session.LoadAsync<EmployeeIssueCommandModel>(id);
            if (savedTemp is null)
            {
